...
mod triggerlike {
    ...
    dyn_clone::clone_trait_object!(Triggerlike);
    dyn_eq::eq_trait_object!(Triggerlike);
    dyn_hash::hash_trait_object!(Triggerlike);

    impl Reflect for Box<dyn Triggerlike> {
        fn get_represented_type_info(&self) -> Option<&'static TypeInfo> {
            Some(Self::type_info())
        }
    ...
    impl Typed for Box<dyn Triggerlike> {
        fn type_info() -> &'static TypeInfo {
            static CELL: NonGenericTypeInfoCell = NonGenericTypeInfoCell::new();
            CELL.get_or_set(|| TypeInfo::Value(ValueInfo::new::<Self>()))
        }
    ...
    impl TypePath for Box<dyn Triggerlike> {
        fn type_path() -> &'static str {
            static CELL: GenericTypePathCell = GenericTypePathCell::new();
            CELL.get_or_insert::<Self, _>(|| {
                {
                    format!("std::boxed::Box<dyn {}::Triggerlike>", module_path!())
                }
            })
        }
        ...
        fn short_type_path() -> &'static str {
            static CELL: GenericTypePathCell = GenericTypePathCell::new();
            CELL.get_or_insert::<Self, _>(|| "Box<dyn Triggerlike>".to_string())
        }

        fn type_ident() -> Option<&'static str> {
            Some("Box<dyn Triggerlike>")
        }
    ...
    impl GetTypeRegistration for Box<dyn Triggerlike> {
        fn get_type_registration() -> TypeRegistration {
            let mut registration = TypeRegistration::of::<Self>();
            registration.insert::<ReflectDeserialize>(FromType::<Self>::from_type());
            registration.insert::<ReflectFromPtr>(FromType::<Self>::from_type());
    ...
    impl FromReflect for Box<dyn Triggerlike> {
        fn from_reflect(reflect: &dyn Reflect) -> Option<Self> {
            Some(reflect.as_any().downcast_ref::<Self>()?.clone())
        }
    }
