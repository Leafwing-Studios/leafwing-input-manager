...
use bevy::{ecs::system::Query, prelude::Res};
...
/// Stateful run condition that can be toggled via an action press using [`ActionState::just_pressed`].
pub fn action_toggle_active_component<A>(
    default: bool,
    action: A,
...
{
    let mut active = default;
    move |action_state: Query<&ActionState<A>>| {
        if let Ok(action_state) = action_state.single() {
            active ^= action_state.just_pressed(&action);
        }
        active
    }
...
/// Run condition that is active if [`ActionState::pressed`] is true for the given action.
pub fn action_pressed_component<A>(action: A) -> impl FnMut(Query<&ActionState<A>>) -> bool
where
    A: Actionlike + Clone,
{
    move |action_state: Query<&ActionState<A>>| {
...
/// Run condition that is active if [`ActionState::just_pressed`] is true for the given action.
pub fn action_just_pressed_component<A>(action: A) -> impl FnMut(Query<&ActionState<A>>) -> bool
where
    A: Actionlike + Clone,
{
    move |action_state: Query<&ActionState<A>>| {
...
/// Run condition that is active if [`ActionState::just_released`] is true for the given action.
pub fn action_just_released_component<A>(action: A) -> impl FnMut(Query<&ActionState<A>>) -> bool
where
    A: Actionlike + Clone,
{
    move |action_state: Query<&ActionState<A>>| {
