...
mod tests {
    ...
    fn durations() {
        ...
        assert_eq!(
            action_state.state(&Action::Jump).instant_started(),
            Some(t0)
        );
        assert_eq!(
            action_state.state(&Action::Jump).current_duration(),
